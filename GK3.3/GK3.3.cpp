// GK3.3.cpp : Defines the entry point for the console application.
//
...
void GenerateTextures()
{
	...
	// wczytanie tekstury Uran utworzonej ze zdjęć sondy Magellan
	...
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku ziemia.tga");
		exit(0);
	}
	...
	error = load_targa("mercury.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku mercury.tga");
		exit(0);
	}
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("Uran", URANUS_TEX);
	glutAddMenuEntry("Ziemia", EARTH_TEX);
	glutAddMenuEntry("Mercury", MERCURY_TEX);
